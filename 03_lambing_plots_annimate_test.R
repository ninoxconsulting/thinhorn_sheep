...
allpts <- allpts |> 
  select("tag_idn", "Latitude", "Longitude", "date_pst.x","date_time_pst", "speed_ave") 
  

# select only 2024 data 
allpts <- allpts %>%
  filter(date_pst.x >= "2024-01-01" & date_pst.x <= "2024-12-31")

unique(allpts$tag_idn)

# select only 2024 data 
allpts <- allpts %>% filter(tag_idn %in% c( 55671) )
...
animated_data <-  ggplot(data, aes(long, lat, color = id)) +
  geom_point(size = 3) +
  theme_minimal() +
